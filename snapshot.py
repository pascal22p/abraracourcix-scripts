...
def waitForOnGoingTask(client, id = None):
    ...
    while waiting:
        ...
        if id is None:
            tasks = client.get('/vps/vps-69c789b2.vps.ovh.net/tasks')
            for task in tasks:
                taskResult = client.get('/vps/vps-69c789b2.vps.ovh.net/tasks/%s'%task)
            ...
            taskResult = client.get('/vps/vps-69c789b2.vps.ovh.net/tasks/%s'%id)
            if taskResult['state']!='done':
                logger.debug("Waiting for task %s (%s) to finish. progress:%d, state:%s"%(taskResult['type'], taskResult['id'], taskResult['progress'], taskResult['state']))
                waiting = True
        depth += 1
...
def deleteSnapshot(client, depth = 0):
    ...
    try:
        result = client.delete('/vps/vps-69c789b2.vps.ovh.net/snapshot')
        return result['id']
...
def main():
    ...
    try:
        result = client.get('/vps/vps-69c789b2.vps.ovh.net/snapshot')
    except ovh.exceptions.ResourceNotFoundError:
    ...
    result = client.post('/vps/vps-69c789b2.vps.ovh.net/createSnapshot', description='Automated snapshot')
    id = result['id']
